    ...
    <section className="py-12 px-4 bg-[#E2D4C0]">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">Don't miss these limited-time offers and discounts!</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {deals && deals.length > 0 ? (
              ...
              >
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="font-bold text-lg text-gray-900 line-clamp-2">{deal.name}</h3>
                    <span className="text-lg font-bold text-blue-600 ml-2">{deal.price}</span>
                  </div>
                  <p className="text-gray-600 text-sm mb-2 line-clamp-2">{deal.specs}</p>
                  {/* Price/Discount */}
                  ...
                  <div className="flex items-center mb-4">
                    <div className="flex text-yellow-400">
                      {[...Array(5)].map((_, i) => (
                        i < Math.floor(deal.rating || 4) ? 
                          <FaStar key={i} className="text-sm" /> : 
                          <FaRegStar key={i} className="text-sm" />
                      ))}
                    </div>
                    <span className="text-sm text-gray-500 ml-2">({deal.reviews || 42})</span>
                  </div>
                  {/* Add to Cart Button */}
                  <button
                  ...
                  {/* Timer */}
                  <div className="text-blue-600 text-xs mt-3 font-semibold">Time left: {deal.timeLeft}</div>
                </div>
                {/* Hover Overlay */}
                {hoveredCard === deal.id && (
            ...
            <div className="col-span-full text-center py-16">
              ...
              <p className="text-gray-600 max-w-md mx-auto">Check back soon for more offers and discounts</p>
            </div>
          )}
        </div>
      </div>
